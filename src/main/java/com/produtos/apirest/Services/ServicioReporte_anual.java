...
import com.produtos.apirest.Services.ServicioReporte_mensual.Reporte_mensualRowMapper;
	...
	public class Reporte_anualRowMapper implements RowMapper<Reporte_anual> {
		...
		public Reporte_anual mapRow(ResultSet rs, int arg1) throws SQLException {
			...
			if(todos)
			{
				...
				r.setReportes_mensuales(servicioReporte_mensual.listarTodosLosMeses(r.getInstitucion().getCod_institucion(), rs.getInt("anio")));
			...
			r.setReportes_mensuales(servicioReporte_mensual.listarTodosLosMesesDeAcuerdoALaInstitucion(r.getInstitucion().getCod_institucion(), rs.getInt("anio")));
	...
	public  Reporte_anual buscar(Reporte_anual re){
		...
		if(re.getInstitucion().getCod_institucion().equals(""))
		{
			...
			System.out.println("estado de todos los akjfajfñajf"+todos);
			 Object datos[]={ re.getAnio()};
				String sql="SELECT  extract(year from s.fecha) as anio ,count(soe.cod_precio_examen) \n" + 
						"as nro_prestaciones_total, sum(pe.costo) as monto_total   FROM sol_exam soe, solicitud s, precio_examen pe   \n" + 
						"WHERE pe.cod_precio_examen=soe.cod_precio_examen  and   soe.cod_sol_exam=(select max(cod_sol_exam) from sol_exam\n" + 
						"																		  where cod_precio_examen=soe.cod_precio_examen and cod_solicitud=soe.cod_solicitud) and soe.cod_solicitud=s.cod_solicitud and pe.cod_institucion ilike '%%'  and extract(year from s.fecha)=?  group by 1 ";
		...
		{
			todos=false;
			Object datos2[]={re.getInstitucion().getCod_institucion(), re.getAnio()};
			String sql2="SELECT  pe.cod_institucion,extract(year from s.fecha) as anio ,count(soe.cod_precio_examen) as nro_prestaciones_total, sum(pe.costo) as monto_total   FROM sol_exam soe, solicitud s, precio_examen pe      WHERE pe.cod_precio_examen=soe.cod_precio_examen  and   soe.cod_sol_exam=(select max(cod_sol_exam) from sol_exam  where cod_precio_examen=soe.cod_precio_examen and cod_solicitud=soe.cod_solicitud) and soe.cod_solicitud=s.cod_solicitud and pe.cod_institucion=?  and extract(year from s.fecha)=?  group by 1, 2 ";

			try {
				reporte=db.queryForObject(sql2,datos2, new Reporte_anualRowMapper());
			} catch (DataAccessException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
